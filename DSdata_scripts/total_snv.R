pdf(file="total_snv.pdf", width=12, height=8)

a<-read.table(file="total_snv.txt", head=T)
library(ggplot2)
library(ggpubr)
...
sig <- stat_compare_means(comparisons = compare, method="t.test", label="p.signif", hide.ns = FALSE,)
...
# Class
#ggplot(a[a$Tools=="LFMD", ], aes(Class, Count, fill=Class )) + geom_boxplot() + mlabs + labs(title="LFMD") + theme

# Label
#ggplot(a[a$Tools=="LFMD", ], aes(Label, Count, fill=Label )) + geom_boxplot() + mlabs + labs(title="LFMD") + theme

ggboxplot(a, x="Label", y="Count", facet.by = "Tissue", fill="Label" ) +  mlabs + sig + theme
#ggboxplot(a[a$Sample != "558H",], x="Label", y="Count", facet.by = "Mutation", fill="Label" ) +  mlabs + sig + theme
ggboxplot(a, x="Label", y="Count", fill="Label") +  mlabs + sig + theme
